...
std::unique_ptr<Pizza> NewYorkStylePizzaStore::createPizza(std::string_view type, bool verbose)
    const
{
...
std::unique_ptr<Pizza> ChicagoStylePizzaStore::createPizza(std::string_view type, bool verbose)
    const
{
    std::unique_ptr<Pizza> pizza;
    if (type == "cheese")
